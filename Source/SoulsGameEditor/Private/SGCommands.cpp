...
#include "SoulsGameEditor/Public/SGCommands.h"
...
TSharedPtr<SWindow> FSGCommands::SGWindow = nullptr;

FSGCommands::FSGCommands()
	: TCommands<FSGCommands>	(TEXT("FSCommands"), NSLOCTEXT("Contexts", "FSCommands", "Souls Game Commands"), NAME_None, TEXT("StyleName"))
{
}

void
FSGCommands::RegisterCommands()
{
	UI_COMMAND(_OpenSGWindow, "OpenSGWindow", "Opens SG Window", EUserInterfaceActionType::Button, FInputChord());
}

void FSGCommands::OpenSGWindow()
{
...
TSharedRef< SVerticalBox > FSGCommands::CreateSGContent()
{
	/*
	...
	*/
	...
	USGEditorInfoObject* EditorObject = GetInfoObject();
...
bool FSGCommands::IsObjectAcceptable(UObject* Asset)
{
...
bool FSGCommands::HasClassName(const FName& Name)
{
	TArray<const UClass*> AllowedClasses = GetAllowedClasses();
...
USGGameSingleton* FSGCommands::GetGameSingleton()
{
	USGGameSingleton* GameSingleton = Cast<USGGameSingleton>(GEngine->GameSingleton);
	return GameSingleton;
}

USGEditorInfoObject* FSGCommands::GetInfoObject()
{
	USGGameSingleton* GameSingleton = GetGameSingleton();
	return GameSingleton->GetEditorObject();
}

void FSGCommands::ForceUpdateWindow()
{
	TSharedRef< SVerticalBox > VerticalBox =  CreateSGContent(); 
	SGWindow->SetContent(VerticalBox);
}

void FSGCommands::OpenWindow(TSharedPtr<SWindow> Window)
{
	if (Window == nullptr)
	{
		return;
...
TArray<const UClass*> FSGCommands::GetAllowedClasses()
{
