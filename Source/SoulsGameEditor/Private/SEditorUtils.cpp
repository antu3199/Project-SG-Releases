...
TSharedPtr<FAssetThumbnail> FSEditorUtils::GetThumbnail(const UObject* ReferenceObject,
...
void FSEditorUtils::CreateAssetPicker(TSharedRef<SVerticalBox>& InVerticalBox,
	...
	const TFunction<bool(UObject*)>& OnSelectObject, const TArray<const UClass*>& AllowedClasses)
{
	...
	[
		...
		.OnAreAssetsAcceptableForDrop_Lambda([AllowedClasses](TArrayView<FAssetData> InAssets)
		{
			...
			return FSEditorUtils::IsObjectAcceptable(Asset, AllowedClasses);
		})
...
void FSEditorUtils::CreateTextBox(TSharedRef<SVerticalBox>& InVerticalBox, IDetailCategoryBuilder& InCategoryBuilder,
...
void FSEditorUtils::CreateButton(TSharedRef<SVerticalBox>& InVerticalBox, IDetailCategoryBuilder& InCategoryBuilder,
...
void FSEditorUtils::ReadFromJson(const FString Path, FSAnimationFileReaderStruct& ResultStruct)
{
	...
	{
		FSEditorUtils::GenerateStructsFromJson(ResultStruct, JsonObject);
	}
	else {
...
void FSEditorUtils::GenerateStructsFromJson(FSAnimationFileReaderStruct& ReaderObject,
                                            TSharedPtr<FJsonObject> JsonObject)
{
	...
	for (int32 i = 0; i < EventsArray.Num(); i++)
	{
		...
		FSAnimationFileReaderEvent Event;
		GenerateStructsFromJson(Event, Json);
		ReaderObject.Events.Add(Event);
	}
}

void FSEditorUtils::GenerateStructsFromJson(FSAnimationFileReaderEvent& ReaderObject,
	TSharedPtr<FJsonObject> JsonObject)
{
	...
	for (int32 i = 0; i < AnimNotifiesArray.Num(); i++)
	{
		...
		TSharedPtr<FJsonObject> Json = Value->AsObject();
		FSAnimationFileReaderAnimNotify AnimNotify;
		GenerateStructsFromJson(AnimNotify, Json);
		ReaderObject.AnimNotifies.Add(AnimNotify);
	}
}

void FSEditorUtils::GenerateStructsFromJson(FSAnimationFileReaderAnimNotify& ReaderObject,
	TSharedPtr<FJsonObject> JsonObject)
{
...
UClass* FSEditorUtils::FindClass(const FString& ClassName)
{
	check(*ClassName);
...
float FSEditorUtils::FramesToTime(float Frames)
{
	return (1 / 30.0f) * Frames;
}

bool FSEditorUtils::IsObjectAcceptable(UObject* Asset, const TArray<const UClass*>& AllowedClasses)
{
...
bool FSEditorUtils::HasClassName(const FName& Name, const TArray<const UClass*>& AllowedClasses)
{
	for (auto & Class : AllowedClasses)
	{
