...
bool FSGUtils::bIsTimestopped = false;

void FSGUtils::DrawSphere(const UWorld* World, FVector Location, float Radius, float Lifetime, FColor Color)
{
    DrawDebugSphere(World, Location, Radius, 32, Color, false, Lifetime);
}

void FSGUtils::DrawBox(const UWorld* World, UBoxComponent* BoxComponent, float Lifetime, FColor Color)
{
    if (World == nullptr)
    {
        return;
...
void FSGUtils::DrawSphere(const UWorld* World, USphereComponent* SphereComponent, float Lifetime, FColor Color)
{
    if (World == nullptr)
    {
        return;
...
void FSGUtils::DrawCapsule(const UWorld* World, UCapsuleComponent* Capsule, float Lifetime, FColor Color)
{
...
void FSGUtils::NormalizeCurveWithDuration(UCurveFloat* Curve, float Duration)
{
}

float FSGUtils::RoundFloatToPrecision(float TheFloat, int32 Precision)
{
    float Pow =  FMath::Pow(10.0f,1 * Precision);
    return FMath::RoundToFloat(TheFloat * Pow) / Pow;
}

FString FSGUtils::GetFloatAsStringWithPrecision(float TheFloat, int32 Precision, bool IncludeLeadingZero)
{
...
void FSGUtils::DoTimeDilationEffect_DEPRECATED(const UObject* WorldContextObject, TArray<AActor*> IgnoreActors)
{
...
void FSGUtils::DoReverseDilationEffect_DEPRECATED(const UObject* WorldContextObject, TArray<AActor*> IgnoreActors)
{
...
void FSGUtils::StopTimeForCharacter(const UObject* WorldContextObject, ASGCharacterBase* Character)
{
    ...
    if (ASGHumanoidCharacter* HumanoidCharacter = Cast<ASGHumanoidCharacter>(Character))
    {
    ...
    FSGUtils::GetAllActorsInWorld(WorldContextObject, TimestopActors, IgnoreActors);
    FSGUtils::StopTimeForActors(WorldContextObject, TimestopActors);
    if (ASGGameModeBase* GameMode = Cast<ASGGameModeBase>(UGameplayStatics::GetGameMode(WorldContextObject)))
    {
...
void FSGUtils::ResumeTimeForCharacter(const UObject* WorldContextObject, ASGCharacterBase* Character)
{
    TArray<AActor*> TimestopActors;
    TArray<AActor*> ChildActors;
    Character->GetAllChildActors(ChildActors);
    ...
    FSGUtils::GetAllActorsInWorld(WorldContextObject, TimestopActors, IgnoreActors);
    FSGUtils::ResumeTimeForActors(WorldContextObject, TimestopActors);
    if (ASGGameModeBase* GameMode = Cast<ASGGameModeBase>(UGameplayStatics::GetGameMode(WorldContextObject)))
    {
...
void FSGUtils::StopTimeForActors(const UObject* WorldContextObject, TArray<AActor*> ActorsToStop)
{
...
void FSGUtils::ResumeTimeForActors(const UObject* WorldContextObject, TArray<AActor*> ActorsToResume)
{
...
void FSGUtils::GetAllActorsInWorld(const UObject* WorldContextObject, TArray<AActor*>& OutActors, const TSet<AActor*>& IgnoreActors)
{
...
bool FSGUtils::GetGroundLocation(const UObject* WorldContextObject, const FVector& Location, FVector& OutLocation)
{
...
void FSGUtils::SetActorEnabled(AActor* Actor, bool bEnabled, bool bModifyTick, bool bDisableChildren)
{
    if (Actor == nullptr)
    {
        return;
...
USpringArmComponent* FSGUtils::GetSpringArmComponent(AActor* Actor)
{
...
void FSGUtils::SetSpringArmLagOnActor(AActor* Actor, bool bLag)
{
...
USGTeamComponent* FSGUtils::GetTeamComponent(AActor* Actor)
{
    ASGCharacterBase* CastedCharacter = Cast<ASGCharacterBase>(Actor);
...
ASGCharacterBase* FSGUtils::GetClosestTeamCharacterFromLocation(const UWorld* World, FVector Location, ESGTeam Team, float MaxDistance,
    bool bCreateDebugSphere, bool bIgnoreDisabledActors)
{
    ...
    else
    {
        ...
        UClass * SeekClass = ASGCharacterBase::StaticClass();
        ...
        {
            FSGUtils::DrawSphere(World, Location, MaxDistance);
        }
    }

    ASGCharacterBase* Character = nullptr;
    float RunningDistance = -1;
    
    for (auto& Actor : CheckedActors)
    {
        if (ASGCharacterBase* CharacterCasted = Cast<ASGCharacterBase>(Actor))
        {
...
void FSGUtils::RotateCharacterToLookTarget(AActor* Actor, const FVector& LookTarget, bool bLockZAxis)
{
    if (Actor == nullptr)
    {
        return;
...
void FSGUtils::GetAllActorsOfStreamingLevel(const UWorld* WorldContextObject, const ULevelStreaming* LevelToStream, const TSubclassOf<AActor> ActorClass, bool bVisibleAfterLoad, TArray<AActor*>& OutActors)
{
    FLatentActionInfo LatentInfo;
