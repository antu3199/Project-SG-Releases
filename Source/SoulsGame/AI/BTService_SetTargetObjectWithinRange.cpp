...
#include "BTService_SetTargetObjectWithinRange.h"
...
UBTService_SetTargetObjectWithinRange::UBTService_SetTargetObjectWithinRange()
{
	NodeName = "UBTService_SetTargetObjectWithinRange";
}

void UBTService_SetTargetObjectWithinRange::TickNode(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	...
	APlayerAIController* AIController = Cast<APlayerAIController>(Character->GetController());
	if (!AIController)
	{
		return;
	}
	...
	*/
	...
	{
		if (APlayerAIController* CastedController = Cast<APlayerAIController>(Character->GetController()))
		{
	...
	if (bPreviouslyHadTarget != bHasTarget)
	{
		...
		if (ASCharacterBase* ClosestCharacter = Cast<ASCharacterBase>(ClosestTarget))
		{
