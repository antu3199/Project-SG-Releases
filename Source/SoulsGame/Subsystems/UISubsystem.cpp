...
void USGUISubsystem::InitializeUIForPlayer(APlayerController* InController, bool bInitializeGameUI)
{
	...
	if (bInitializeGameUI)
	{
		...
		{
			HUDInstance = Cast<UHUDWidget>(CreateWidget(InController, Settings->HUDClass));
		}
	}
}
...
void USGUISubsystem::AddHUDToViewport()
{
	...
	{
		if (const ASCharacterBase* Character = Cast<ASCharacterBase>(Controller->GetCharacter()))
		{
			HUDInstance->InitializeHUD(Character->GetAttributeSet());
		}
