#include "SGFloatingCombatTextComponent.h"
...
USGFloatingCombatTextComponent::USGFloatingCombatTextComponent()
{
}

void USGFloatingCombatTextComponent::BeginPlay()
{
	Super::BeginPlay();
}

void USGFloatingCombatTextComponent::BeginDestroy()
{
	Super::BeginDestroy();
}

void USGFloatingCombatTextComponent::TickComponent(float DeltaTime, ELevelTick TickType,
                                                 FActorComponentTickFunction* ThisTickFunction)
{
	...
	for (int32 i = 1; i < ActiveTextActors.Num(); i++)
	{
		...
		ASGFloatingTextActor* TextActor = ActiveTextActors[i];
...
void USGFloatingCombatTextComponent::AddFloatingText_Client_Implementation(const FText& Text, FVector WorldLocation, const FColor& Color)
{
	...
	ASGFloatingTextActor* NewTextActor = World->SpawnActorDeferred<ASGFloatingTextActor>(FloatingTextActorClass, SpawnTransform, Owner);
	...
	NewTextActor->OnDestroyed.AddDynamic(this, &USGFloatingCombatTextComponent::OnTextDestroyed);
...
void USGFloatingCombatTextComponent::OnTextDestroyed(AActor* DestroyedActor)
{
