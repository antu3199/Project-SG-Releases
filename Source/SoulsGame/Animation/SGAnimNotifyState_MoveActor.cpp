#include "SGAnimNotifyState_MoveActor.h"

#include "Components/CapsuleComponent.h"
#include "GameFramework/CharacterMovementComponent.h"
#include "SoulsGame/UE4Overrides/SGCharacterMovementComponent.h"

USGAnimNotifyState_MoveActor::USGAnimNotifyState_MoveActor()
{
}

void USGAnimNotifyState_MoveActor::DoNotifyBegin(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation, float TotalDuration)
{
	...
	else
	{
		...
		{
			USGCharacterMovementComponent* MovementComponent = Character->GetCharacterMovementComponent();
	...
	if (bChangeCollisionProfileType)
	{
		...
		if (ASGCharacterBase* Character = Cast<ASGCharacterBase>(Actor))
		{
...
void USGAnimNotifyState_MoveActor::DoNotifyEnd(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation)
{
...
void USGAnimNotifyState_MoveActor::NotifyTick(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation, float FrameDeltaTime,
...
FVector USGAnimNotifyState_MoveActor::GetCurrentVelocity() const
{
	...
	if (ASGCharacterBase* Character = Cast<ASGCharacterBase>(CachedActor))
	{
...
FVector USGAnimNotifyState_MoveActor::GetRelativeDirection(AActor* Actor) const
{
	if (IsPreviewingInEditor())
	{
		return Direction;
